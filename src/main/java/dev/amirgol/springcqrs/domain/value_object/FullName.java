...
public record FullName(@NotNull String value) {
    ...
    public FullName {
        ...
        if (!FULLNAME_PATTERN.matcher(value.trim()).matches()) {
            throw new ValidationException("Full name does not meet required format. Ensure it contains only letters, spaces, and follows standard naming conventions. Provided: '" + value + "'");
        }
